 ...
 * 감상 렌즈라 ViewerScreen 아래에 놓인다(렌즈 탭 바는 ViewerScreen이 소유).
 * 정리는 폴더 분류에서. */
...
/** 사람/장소/비디오 렌즈 본문. lens는 ViewerScreen의 렌즈 바에서 정해지고,
 * 사람 그룹 드릴은 store의 groupId(뒤로가기 통합)로 관리한다. */
export function TagLensBody({ lens }: { lens: "people" | "places" | "videos" }) {
  ...
  return (
    ...
    <div className="h-full">
      ...
      {lens === "places" && <PlacesRegionView space={space} />}
      {lens === "videos" && <VideosGrid space={space} />}
    </div>
  );
}
