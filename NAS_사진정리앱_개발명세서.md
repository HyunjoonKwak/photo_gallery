...
### 9.1 공통 셸
- 상단바: 라이브러리 셀렉터(📚공용/👤내 사진/(관리자)구성원/📦1차 구역 — 1차
  구역엔 신규 유입 +N 뱃지) · 주 메뉴 3영역(사진/앨범/폴더 분류) · 검색창
  (1차 구역·타인 열람 중엔 숨김) · 빌드 배지 · 작업 기록 · 로그아웃.
...
- 렌즈 5종: 🕑타임라인(연/월/일 줌 바둑판, 사이드 스크러버, 연뷰 사진 탭→해당
  월 정밀 안착+상단 크럼이 실제 위치 추종) · 🗂폴더(계층 탐색+정사각 그리드) ·
...
### 9.4 폴더 분류 (정리)
- Finder식 커맨더: 단일/분할 페인, 페인별 영역(공용/내사진/1차구역/타인) 선택.
...
- 감상: 앨범 담기만. 폴더 분류: 이동/삭제(Delete) 포함.
...
- 구역 관리: 1차 구역 등록/삭제(홈 하위 FileStation 탐색), 열람 시 신규 유입
  뱃지 리셋.
