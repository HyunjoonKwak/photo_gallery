...
> **⚠️ 현행 IA(2026-07-06 개편, 2026-07-11 확정 기록)** — 아래 원문(9.1~9.4)은 초기 설계이며, 실제 구현은 다음 구조다. 충돌 시 이 블록이 우선한다.
> - **상위 3영역**: 사진(감상) / 앨범 / 폴더 분류(정리). 상단(데스크톱)·하단 탭(모바일).
> - **사진(감상)**: 렌즈 5종 — 타임라인(연/월/일 줌) · 폴더 · 사람 · 장소(지역+지도) · 비디오. 순수 뷰어(라이트박스는 열람 전용).
> - **폴더 분류(정리)**: Finder식 단일/분할 커맨더(FolderPane×2), DnD/액션바/충돌 처리/일괄 진행바, 중복 정리 탭. 1차 구역·타인 라이브러리는 여기서만.
> - **라이브러리 셀렉터**(좌상단): 공용/내 사진/(관리자)구성원/1차 구역 전환.
> - **검색**: 상단 검색창(파일명·폴더명·태그, DSM 인덱스) → 검색 뷰. 1차 구역·타인 열람 중에는 숨김.
> - **드래그 박스 선택은 미구현으로 확정**(2026-07-11 결정) — 터치 우선 UX로 길게 눌러 선택+Shift 범위로 충분, 가상화 그리드와의 충돌 회피. 원문 3장의 해당 항목은 폐기.
