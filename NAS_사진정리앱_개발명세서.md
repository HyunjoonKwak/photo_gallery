...
## 9. 화면 명세 (현행 — 2026-07-12 전면 갱신)

> 2026-07-06 IA 개편 이후의 실제 구현을 기준으로 재작성. 초기 설계(공용/개인
> 탭 + 타임라인/폴더 토글)는 폐기됐다. 이력은 docs/IMPROVEMENTS.md 참고.

### 9.1 공통 셸
> 앱 표시명(2026-07-25 사용자 확정): **우리집 사진관** (PWA short_name **사진관**)
> — 헤더 h1·로그인·`<title>`·매니페스트 name/short_name·iOS 홈 이름 일괄 적용.
> 1줄 오른쪽에 **로그인 계정 칩**(👤 계정명·관리자 뱃지, 클릭=더보기) 상시 표시.
> 2026-07-25 용어 개편: 공용→**가족**, 1차 구역→**기기 백업**, 폴더 분류→**정리**,
> 폴더 렌즈→**폴더별 보기**, 마법사 탭→**정리 도우미**. 내부 코드명(team/zone/
> manage)은 유지 — UI 라벨만 사용자 언어로 변경.
- 상단바(2026-07-25 **2줄 구조 A안** — 두 축 시각 분리): **1줄** = 라이브러리
  셀렉터(제목 스타일 — 📚가족/👤내 사진/(관리자)구성원/📦기기 백업, 신규 유입
  +N 뱃지; 기기 백업=인디고 필·타인=주황 필로 상태 유지) + 검색창(기기 백업·
  타인 열람 중엔 **비활성+이유 툴팁**, 숨기지 않음) + MOCK 표식. **2줄** =
  4영역 **언더라인 탭**(사진/앨범/정리/더보기, md 이상 — 모바일은 하단 탭 바가
  담당하므로 모바일 헤더는 1줄만). "무엇을(제목)"과 "무엇을 할지(탭)"가 다른
  시각 언어를 갖게 해 라이브러리가 다섯 번째 탭으로 오독되는 것을 방지.
  앨범 영역에선 셀렉터가 "내 사진" 고정 표시(개인 전용이라 죽은 컨트롤 방지).
- 셀렉터 예측 가능성(2026-07-25 3단계): 기기 백업·구성원 항목은 **모든 영역에서
  상시 노출**("정리에서 열림" 힌트). 감상 중 선택 시 예고 토스트 후 정리로 이동,
  감상 영역 복귀로 선택이 풀릴 땐 "내 사진으로 돌아왔어요" 토스트(암묵 전환 5건
  전부 예고/알림 동반).
- **더보기(⋯) 허브**: 계정 카드(로그아웃) · 🗑휴지통(일급 승격 — 작업 기록
  패널을 휴지통 펼침 상태로 오픈) · 🕘작업 기록 · 📲기기 백업 관리(구 구역
  관리, 라이브러리 드롭다운에서 이사) · 🖥DSM 연결 정보 · 🧪빌드·진단(구
  헤더 빌드 배지). 더보기 진입은 기기 백업/타인 선택을 풀지 않는다(스냅백 예외).
- 모바일: 하단 탭 바(4영역), 시스템 뒤로가기는 히스토리 깊이 미러링(useBackTrap).
- 첫 로그인 1회 안내 카드(FirstRunTip): 2축 구조(무엇을×어떻게)+되돌리기 안전망
  소개, localStorage로 재표시 안 함. 타임라인 빈 상태에 다음 행동 안내문.
- 다이얼로그는 공용 Modal(askConfirm/askPrompt) — window.prompt/confirm 금지.

### 9.2 사진 (감상 — 순수 뷰어)
- 렌즈 5종: 🕑타임라인(연/월/일 줌 바둑판, 사이드 스크러버, 연뷰 사진 탭→해당
  월 정밀 안착+상단 크럼이 실제 위치 추종) · 📂폴더별 보기(계층 탐색+정사각 그리드) ·
  👤사람(이름 지정=동명 병합, 중복 병합) · 📍장소(국가→지역+지도(leaflet)) ·
  🎬비디오.
- 모든 그리드는 행 단위 가상화(보이는 행만 마운트).
- 감상에서의 편집은 라이트박스 '앨범' 담기뿐(이동/삭제 없음).

### 9.3 앨범
- DSM NormalAlbum 연동: 생성(빈/담으며)/열람/담기/이름변경(✎)/사진 빼기(빼기
  모드)/삭제. 개인 공간 전용.

### 9.4 정리 (구 "폴더 분류")
- 탭 순서 = 위계(2026-07-25): 📁폴더 · ✨정리 도우미(대표 정리 흐름) · 🔁중복
  정리(고급 도구 — 도우미 1단계와 같은 엔진, 가족 공간 지원 툴팁 명시).
- 검색 결과 진입 시 컨텍스트 칩(🔍 "검색어" 결과 + ✕=이전 화면 복귀).
- Finder식 커맨더: 단일/분할 페인, 페인별 영역(가족/내사진/기기백업/타인) 선택.
- 선택 모델: 클릭/Shift 범위/전체 선택/**마우스 러버밴드**(빈 공간 드래그, Shift=추가, 가상화 화면 밖 포함 — 2026-07-12) — 하단 액션바(이동/복사/공용에 복사/
  앨범에 추가/삭제). DnD 이동(⌥=복사), 스프링 드릴인.
- 대량 작업: 500장 자동 분할 + 진행바(장/%), 이름 충돌은 전역 다이얼로그
  (건너뛰기/이름변경/덮어쓰기/합치기), 전부 되돌리기 가능(휴지통+작업로그).
- 폴더 도구: 새 폴더/이름 변경/폴더째 이동·복사/삭제(휴지통)/이름 정리(날짜부
  밑줄→하이픈)/촬영일 교정(1차구역=파일 기록, 내사진·공용=Foto set, 재귀+수동).
- 중복 정리 탭: 유사도 슬라이더, 정확 중복 필터, 다중 보관 선택.

### 9.5 라이트박스
- ←/→·스와이프·핀치줌·더블탭, i=정보(EXIF/위치/폴더), 동영상 재생.
- 감상: 앨범 담기만. 정리: 이동/삭제(Delete) 포함.
- 원본 로컬 저장: 지원 브라우저(Chrome/Edge 데스크톱)는 **저장 위치 선택**
  다이얼로그(File System Access API, 스트리밍 저장) + 완료 토스트, 그 외는
  기본 다운로드 폴더 폴백(Android는 Chrome "다운로드 위치 확인" 설정으로 선택).

### 9.6 관리 부속
- 작업 기록 패널: 작업별 되돌리기(7일), 휴지통 요약+내용 보기(사진 단위 선택
  복원), 비우기(관리자 전용·타인 열람 중 차단).
- **계정별 열람(2026-07-25)**: 작업 기록·휴지통은 본인 작업만 표시(남의 개인
  공간 파일명 유출 방지), 되돌리기·선택 복원도 본인 작업만(403). 관리자는
  전체 열람·되돌리기 가능(B-7 감독 모델). 비우기는 여전히 전역+관리자 전용.
- 기기 백업 관리(구 구역 관리): 등록/삭제(홈 하위 FileStation 탐색), 열람 시 신규 유입
  뱃지 리셋.
