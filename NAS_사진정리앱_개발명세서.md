...
- **Frontend**: **React + Vite + TypeScript**, **Tailwind**
  - 드래그앤드롭: `@dnd-kit/core`
  - 가상 스크롤: `@tanstack/react-virtual` (또는 react-window)
...
- (관리자만) **가족 구성원 선택** 드롭다운 + 남의 폴더 열람 시 "보는 중: ○○의 개인 폴더" 배너
- 우상단 보기 토글: **[타임라인] [폴더]**
...
- EXIF 패널: 촬영일시, 카메라/렌즈, 해상도, 용량, 경로/공간
- 라이트박스에서도 이동/복사/삭제/공용으로 보내기 가능

상호작용 규칙
- 다중 선택: 클릭 토글 / Shift 범위 / 드래그 박스
- 드래그 시작 시 "n장" 고스트, 유효 드롭 대상 하이라이트
- 파괴적 작업은 토스트로 "n장 이동됨 · [되돌리기]" 즉시 노출(확인 팝업 최소화, 대신 Undo로 안전 확보)
...
- **2단계**: 중복 사진 탐지/제거 — 정확 중복(체크섬) + 유사 사진(perceptual hash, `imagehash`). 그룹으로 보여주고 "최적 1장만 남기기" 제안, 삭제는 휴지통+Undo
