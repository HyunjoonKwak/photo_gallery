...
- [x] **정리 워크플로우(마법사)** — **Phase 0~3 구현·배포 완료(fb1db04, 2026-07-12)**. 미결 확정: 처분 폴더 `_정리/<사유>`·장소 힌트 v1 제외·최소 8장 슬라이더·진입 `✨ 정리`(관리 탭). no_exif 룰은 photo_cache.camera 전 행 NULL이라 v1 제외. 실 NAS 검증: 후보 755장(스크린샷 724·메신저 31)·이벤트 제안 562건·세션 이어하기/리셋. 잔여 개선 후보: 이벤트 카드 목록 가상화(562카드 전량 마운트), 클러스터 '이미 복사됨' 제외(v2), 장소 힌트 프로브. (원 설계: — 4기기 유입 개인 사진을 중복→잡동사니→이벤트 공용 앨범화로 정리하는 반복 가능 흐름. `docs/ORGANIZE_WIZARD.md`)
