...
- [x] **중복 그룹 카드 비교성 강화** (2026-07-03, 사용자 요청) — ① 각 사진에 **폴더 경로** 표시: photo_cache가 파일명만 저장하므로 groups 응답 시 상위 N개 그룹 아이템만 `item_folders`(Browse.Item get 배치, ~100개/콜)로 온디맨드 해석(스캔 재실행 불필요, 실패 시 폴더만 공백). ② **사진 클릭 = 라이트박스 크게 비교**(그룹 사진들만 ←/→ 순회, `i` EXIF/폴더, Delete 휴지통) — 보관본 변경은 별도 [보관] 배지 클릭으로 분리(클릭 충돌 해소). ③ 카드에 파일명·해상도·용량·**촬영일**·폴더 메타 표기
