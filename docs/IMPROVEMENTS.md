...
- [ ] 3단계(AI 자동 분류) — 미착수. **방향 결정(2026-07-02)**: ① 기본 = Synology Photos 내장 AI 재활용(`SYNO.Foto.Browse.Person`(얼굴)·`Geocoding`(장소) 등을 기존 DSM 클라이언트로 읽어 "인물/장소별 폴더 모으기" 제안 → 실행은 기존 이동+Undo 재사용; 외부 API·추가 비용·NAS 부하 없음, 아키텍처 원칙과 일치). ② 옵트인 = 클라우드 비전 API(Claude 등)로 대표 사진만 보내 폴더명/그룹 제안(API 키 .env 옵트인, 전량 전송 금지). 로컬 ONNX 모델(Immich 방식)은 비권장(컨테이너 비대+NAS CPU 부담 대비 이득 적음). **착수 첫 작업: 실 NAS에서 Person/Geocoding API 프로브**(Photos '인물' 기능 활성 필요)
