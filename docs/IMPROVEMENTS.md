...
- [x] **폴더 이동 충돌 처리 (2026-07-05, 사용자 요청)** — 폴더째 이동 시 대상 위치에 같은 이름 폴더가 있으면 CopyMove가 overwrite=false로 **조용히 건너뛰고 성공 보고**해 "이동했다는데 아무 변화 없음" 발생. 사진과 동일한 강제 패턴으로 해결: `MoveFoldersRequest.conflict_strategy` 기본 `ask` → `execute_move_folders`가 `folder_conflicts()`로 감지해 **409+목록**(`{code:folder_conflict, conflicts:[{folder_id,name}]}`). 프론트 `moveFoldersMutation.onError`가 전역 `ConflictDialogHost`(파일/폴더 겸용으로 일반화)를 띄우고 선택 전략으로 재시도. 폴더 전략은 **rename**(`name_1` — DSM은 temp 경유 rename dir) / **skip**만(overwrite는 전체 트리 소실이라 제외). `_list_subdir_names`로 대상 하위 폴더명 조회. mock 시뮬레이션 + pytest(ask→409/rename) + mock UI e2e(분할 뷰 폴더 이동→409→다이얼로그→`가족앨범/행사_1`). **남은 것**: 실 NAS 검증. **미구현(향후)**: 폴더 merge(내용 합치기) — 전체 중복일 때 이상적이나 내부 파일 재귀 충돌 처리가 복잡 + 실 NAS 검증 필요
