...
- [x] **1차 구역 썸네일 화질·동영상 미표시 (2026-07-07, 사용자 보고)** — 실 NAS 조사로 근본 원인 확정: 파일 옆 `@eaDir/<name>/`에는 사진·동영상 **모두** 미디어 인덱스 썸네일 `SYNOPHOTO_THUMB_{SM,M,XL}.jpg`가 있는데, 앱이 쓰던 `SYNO.FileStation.Thumb`는 **소형 `SYNOFILE_THUMB`만 서빙**한다. 사진엔 SYNOFILE이 있어 저해상으로 뜨고(일부 폴더는 SYNOFILE만 있어 더 나쁨), **동영상엔 SYNOFILE이 없어 404 → ▶ 폴백(썸네일 미표시)**. **해결**: `HomesPhotoSource/ZonePhotoSource`(FileStation 경로형 id)의 `thumbnail()`을 `@eaDir/<name>/SYNOPHOTO_THUMB_{M|XL}` 직접 다운로드(`SYNO.FileStation.Download`)로 전환 — 사진·동영상 공통 해결 + 화질↑(그리드 M=320px, 라이트박스 XL). SYNOPHOTO가 없는 파일만 기존 `FileStation.Thumb`로 폴백(무회귀). 캐시 키에 버전(`v2`) 추가해 옛 저화질 캐시 무효화. **검증**: pytest 149 그린, import/빌드 통과. **남은 것(실 NAS 검증)**: `FileStation.Download`가 `@eaDir` 경로를 서빙하는지(권한/숨김폴더) — 안 되면 폴백으로 현상 유지되며, 대안(SYNOFILE 썸네일 생성/다른 API) 검토.
